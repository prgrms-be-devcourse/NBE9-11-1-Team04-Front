...
export default function ProductCard({ product, onAddToCart, onProductClick }: { product: Product, onAddToCart: (product: Product) => void, onProductClick: (product: Product) => void }) {
  const handleBtnClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    onAddToCart(product); // 부모의 상태 변경 함수 호출
  };
  ...
  return (
    <div
      ...
      className="bg-white rounded-xl p-4 shadow-sm h-full flex flex-col hover:shadow-md transition-shadow group">
